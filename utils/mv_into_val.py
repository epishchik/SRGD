...
def main() -> None:
    ...
    val_names = np.random.choice(
        names,
        size=int(args.test_size * len(names)),
        replace=False
    )
    ...
    for name in val_names:
        os.system(
            f'mv {args.folder}/270p/{name} {args.folder}_val/270p/{name}'
        )
        os.system(
            f'mv {args.folder}/360p/{name} {args.folder}_val/360p/{name}'
        )
        os.system(
            f'mv {args.folder}/540p/{name} {args.folder}_val/540p/{name}'
        )
        os.system(
            f'mv {args.folder}/1080p/{name} {args.folder}_val/1080p/{name}'
        )
