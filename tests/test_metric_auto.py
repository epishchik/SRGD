...
class MetricAutoTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        map_metric_names = {
            "psnr": "psnr",
        ...
        cls.metric_calculator = MetricSR(
            metric_names,
            no_ref_metric_names,
            map_metric_names,
        )
    ...
    def test_single_image_perfect(self):
        ...
        self.metric_calculator.calculate(ups_hr, real_hr)

        best_psnr, best_ssim, best_lpips = 80.0, 1.0, 0.0

        assert self.metric_calculator.calculate_total("psnr") - best_psnr <= 1e-6
        assert self.metric_calculator.calculate_total("ssim") - best_ssim <= 1e-6
        assert self.metric_calculator.calculate_total("lpips") - best_lpips <= 1e-6
